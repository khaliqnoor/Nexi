...
const StoryModel = ({setShowModel, fetchStories}) => {
    const bgColors = ["#6366F1",  "#33FF57", "#3357FF","#FF33A8",  "#FFD433",  "#33FFF3", ]
    ...
    const handleMediaUpload = (e)=>{
        ...
        if(file){
            setMedia(file)
            setPreviewUrl(URL.createObjectURL(file))   //this is the temporary place if we wanna see the file before uploading like it wont upload to anything else it would just show to you 
        }
    }
    ...
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4  '>
        <div className='w-full max-w-md'>
          ...
          <div className='rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor: background}}>
           {mode === 'text' && (
                <textarea value={text} className='focus:outline-none bg-transparent text-white w-full h-full p-6 text-lg resize-none' placeholder="What's on your Mind today?" onChange={(e)=> setText(e.target.value)} />
           )}
           {
            ...
            <div className='flex gap-2 mt-4'>
                    <button onClick={()=> {setMode('text'); setMedia(null); setPreviewUrl(null)}} className={`flex-1 flex cursor-pointer items-center justify-center gap-2 p-2 rounded ${mode === 'text' ? "bg-white text-black" : "bg-zinc-800"}`}>
